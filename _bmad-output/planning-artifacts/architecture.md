...
| V4 | Stream offset advancement | Zero-row INSERT pattern within explicit transaction — `INSERT INTO _staging.stream_offset_log SELECT * FROM <stream> WHERE 0 = 1` | Vision §8.0 |
...
| OTLP endpoint URL | `_internal.config` (key: `otlp_endpoint`) | Query config table at pipeline startup |
...
| Batch size, intervals | `_internal.config` (key: `export_batch_size`, etc.) | Query config table; hardcoded defaults for MVP |
  ...
  → Stream (APPEND_ONLY)
    → Triggered task fires (SYSTEM$STREAM_HAS_DATA)
      → event_table_collector SP:
        ...
        7. Zero-row INSERT (stream consumed)
        8. COMMIT
